...
export async function GET() {
  try {
    ...
    if (isPostgresAvailable()) {
      methods = await dbPostgres.deliveryMethods.getAll();
    } else {
      methods = db.deliveryMethods.getAll();
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    let created;
    if (isPostgresAvailable()) {
      created = await dbPostgres.deliveryMethods.create(method);
    } else {
      created = db.deliveryMethods.create(method);
    }
